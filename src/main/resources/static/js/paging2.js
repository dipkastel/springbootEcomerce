...
(function($) {
  ...
  $.fn.JPaging = function(param) {
    ...
    // $("<div id='paging'></div>").insertAfter($('#tab-1'));

    var $countLi = $(this).find("span").length;
    ...
    if ($pageCount >= 1 && $visiblePageSize >= 1) {
      $("#paging").append(
          "<a href='javascript:void(0)' style='font-weight:700;'>" + "قبلی"
      );
      ...
      if ($pageCount < $visiblePageSize){
        for (var i = 0; i <= $pageCount; i++) {
          if (i <= $visiblePageSize) {
            if (i==0){
              $("#paging").append("<a href='javascript:void(0)' style='display: none'>" + i + "</a>");
            }else{
      ...
      $("#paging").append(
          "<a href='javascript:void(0)' style='font-weight:700;'>" + "بعدی"
      );

      $("div#" + $thisId + " span:gt(" + ($pageSize - 1) + ")").hide();
    ...
    $("#pre_point").on("click", function(event) {
      ...
      $("div#" + $thisId + " span").hide();
      for (var i = gt - $pageSize; i < gt; i++) {
        $("div#" + $thisId + " span:eq(" + i + ")").show();
      }
